...
class ObjectDetection:
    def __init__(self):
        ...
        self.loop = asyncio.get_running_loop()
        self._postprocess_pool = ThreadPoolExecutor(max_workers=2)
        ...
        print(f"STARTUP: batch_wait=0.01s, overlap_postprocess=True, num_cpus=2, max_batch=10, max_ongoing=100, device={self.device}")

    def _postprocess_batch(self, results, batch_size, t_inf_start):
        """Runs on a separate CPU thread so GPU can start the next batch."""
        ...
        print(f"STAGES batch={batch_size} | postprocess={post_ms:.1f}ms")
        return output
    ...
    async def detect(self, image_urls: list[str]):
        ...
        t0 = time.perf_counter()
        results = await self.loop.run_in_executor(None, self.model, image_urls)
        if self.device.type == "cuda":
            torch.cuda.synchronize()
        t1 = time.perf_counter()
        ...
        print(f"STAGES batch={batch_size} | inference={inf_ms:.1f}ms")

        output = await self.loop.run_in_executor(
            self._postprocess_pool, self._postprocess_batch, results, batch_size, t0
        )
        return output
    ...
    async def detect_bytes(self, image_bytes_list: list[bytes]):
        ...
        if self.device.type == "cuda":
            torch.cuda.synchronize()
        results = await self.loop.run_in_executor(None, self.model, images)
        if self.device.type == "cuda":
            torch.cuda.synchronize()
        t2 = time.perf_counter()
        ...
        self._inference_hist.observe(inf_ms)
        self._gpu_active_ms.inc(inf_ms)
        print(f"STAGES batch={batch_size} | preprocess={pre_ms:.1f}ms | inference={inf_ms:.1f}ms")
